...
namespace ZigBeeNet.Hardware.EmberV8Plus
{
	public class ZigBeeDongleEzsp : IZigBeeTransportTransmit, IEzspFrameHandler
	{
		...
		*/
		public ZigBeeDongleEzsp(string portname, EmberSerialProtocol protocol, ILoggerFactory loggerFactory)
		{
			this._protocol = protocol;
			this._logger = loggerFactory.CreateLogger<ZigBeeDongleEzsp>();

			if (protocol == EmberSerialProtocol.Serial)
			{
				this._ashHost = new AshHost(portname, 115200, loggerFactory, AshVersion.V2);
			}
			else
			{
		...
		*/
		...
		{
			_stackConfiguration.TryGetValue(configId, out ushort previousValue);
			if (value == null)
		...
		*/
		...
		public void SetDefaultDeviceId(ushort defaultDeviceId)
		{
		...
		public ZigBeeStatus Startup(bool reinitialize)
		{
			...
			// Add the endpoint
			_logger.LogDebug("EZSP Adding Endpoint: ProfileID={ProfileID}, DeviceID={DeviceID}",
				_defaultProfileId.ToString("X4"), _defaultDeviceId.ToString("X4"));
			...
			if (reinitialize)
			{
				...
				if (_deviceType == DeviceType.COORDINATOR)
					...
					netInitialiser.JoinNetwork(_networkParameters, _linkKey,
						ZigbeeLeaveNetworkOption.SL_ZIGBEE_LEAVE_NWK_WITH_NO_OPTION);
			}
			...
			_logger.LogDebug("EZSP Current status {status} and Security State = {CurrentSecurityState}",
				currentSecurityState.Status, currentSecurityState.State);

			Status txPowerResponse = ncp.SetRadioPower(_networkParameters.radioTxPower);
			if (txPowerResponse != Status.SL_STATUS_OK)
			{
				_logger.LogDebug("Setting TX Power to {TxPower} resulted in {Response}",
					_networkParameters.radioTxPower, txPowerResponse);
			}
			...
			bool joinedNetwork = (networkState == ZigbeeNetworkStatus.SL_ZIGBEE_JOINED_NETWORK ||
								networkState == ZigbeeNetworkStatus.SL_ZIGBEE_JOINED_NETWORK_NO_PARENT);
			_initialised = true;
		...
		*/
		public void SendCommand( /*int msgTag,*/ ZigBeeApsFrame apsFrame)
		{
			...
			Task.Run(() =>
			{
				try
				{
					...
					{
						_logger.LogDebug("Unable to get response from {Request} :: {Response}",
							transaction.GetRequest(), transaction.GetResponse());
						return;
					}
		...
		public void SetNodeDescriptor(IeeeAddress ieeeAddress, NodeDescriptor nodeDescriptor)
		{
			...
			_logger.LogDebug("{IeeeAddress}: NodeDescriptor passed to Ember NCP {NodeDescriptor}", ieeeAddress,
				nodeDescriptor);
			if ((nodeDescriptor.MacCapabilities & NodeDescriptor.MacCapabilitiesType.RECEIVER_ON_WHEN_IDLE) == 0)
			{
				EmberNcp ncp = GetEmberNcp();
				ncp.SetExtendedTimeout(ieeeAddress.AsByteArray(), true);
			}
		}
		...
		public void HandleLinkStateChange(bool linkState)
		{
			...
			Task.Run(() =>
			{
				...
				_zigbeeTransportReceive.SetTransportState(linkState
					? ZigBeeTransportState.ONLINE
		...
		public ExtendedPanId ExtendedPanId => new(_networkParameters.extendedPanId);
		...
		public ZigBeeKey? ZigBeeNetworkKey
		{
			...
			{
				EmberNcp ncp = GetEmberNcp();
				ZigbeeSecManContext context = new ZigbeeSecManContext
				{
					core_key_type = ZigbeeSecManKeyType.SL_ZB_SEC_MAN_KEY_TYPE_NETWORK,
					key_index = 0,
					derived_type = ZigbeeSecManDerivedKeyType.SL_ZB_SEC_MAN_DERIVED_KEY_TYPE_NONE,
					flags = ZigbeeSecManFlags.ZB_SEC_MAN_FLAG_NONE
				};
				(Status Status, ZigbeeSecManKey Key) result = ncp.SecManExportKey(context);
				if (result.Status == Status.SL_STATUS_OK) return new ZigBeeKey(result.Key.key);
				_logger.LogDebug("Failed to get network key: {Status}", result.Status);
				return null;
		...
		public ZigBeeKey? TcLinkKey
		{
			get
			{
				...
				ZigbeeSecManContext context = new ZigbeeSecManContext
				{
					core_key_type = ZigbeeSecManKeyType.SL_ZB_SEC_MAN_KEY_TYPE_TC_LINK,
					key_index = 0,
					derived_type = ZigbeeSecManDerivedKeyType.SL_ZB_SEC_MAN_DERIVED_KEY_TYPE_NONE,
					flags = ZigbeeSecManFlags.ZB_SEC_MAN_FLAG_NONE
				};
				(Status Status, ZigbeeSecManKey Key) result = ncp.SecManExportKey(context);
				if (result.Status == Status.SL_STATUS_OK) return new ZigBeeKey(result.Key.key);
				_logger.LogDebug("Failed to get TC link key: {Status}", result.Status);
				return null;
				...
				try
				{
					switch (option)
					{
						...
						case TransportConfigOption.INSTALL_KEY:
							...
							ZigbeeSecManKey secManKey = new ZigbeeSecManKey { key = nodeKey.Key };
							Status result = ncp.SecManImportTransientKey(nodeKey.address.AsByteArray(), secManKey);
						...
						case TransportConfigOption.SUPPORTED_INPUT_CLUSTERS:
							configuration.SetResult(option,
								SetSupportedInputClusters((ICollection<ushort>)configuration.GetValue(option)));
							break;

						case TransportConfigOption.SUPPORTED_OUTPUT_CLUSTERS:
							configuration.SetResult(option,
								SetSupportedOutputClusters((ICollection<ushort>)configuration.GetValue(option)));
							break;
		...
		{
			// In V8Plus, trust center policy uses bitmask values instead of simple decision IDs
			// The bitmask values need to be cast to ZigbeeEzspDecisionId for the SetPolicy call
			ZigbeeEzspDecisionId emberJoinMode;
			switch (joinMode)
			{
				case TrustCentreJoinMode.TC_JOIN_INSECURE:
					// Allow joins and send network key in the clear (insecure)
					emberJoinMode = (ZigbeeEzspDecisionId)(
						ZigbeeEzspDecisionBitmask.SL_ZIGBEE_EZSP_DECISION_ALLOW_JOINS |
						ZigbeeEzspDecisionBitmask.SL_ZIGBEE_EZSP_DECISION_SEND_KEY_IN_CLEAR);
					break;
				case TrustCentreJoinMode.TC_JOIN_SECURE:
					// Allow joins with preconfigured keys (secure)
					emberJoinMode = (ZigbeeEzspDecisionId)ZigbeeEzspDecisionBitmask.SL_ZIGBEE_EZSP_DECISION_ALLOW_JOINS;
					break;
				case TrustCentreJoinMode.TC_JOIN_DENY:
					// Disallow all joins and rejoins
					emberJoinMode = (ZigbeeEzspDecisionId)ZigbeeEzspDecisionBitmask.SL_ZIGBEE_EZSP_DECISION_BITMASK_DEFAULT_CONFIGURATION;
					break;
				default:
					return ZigBeeStatus.INVALID_ARGUMENTS;
			}

			return (GetEmberNcp().SetPolicy(ZigbeeEzspPolicyId.SL_ZIGBEE_EZSP_TRUST_CENTER_POLICY, emberJoinMode) ==
					Status.SL_STATUS_OK)
				? ZigBeeStatus.SUCCESS
				: ZigBeeStatus.FAILURE;
		}

		private async Task<bool> InitialiseEzspProtocol()
		{
			if (_frameHandler != null)
			{
			...
			if (!await _ashHost.ConnectAsync())
			{
			...
			switch (_protocol)
			{
				case EmberSerialProtocol.Serial:
			...
			*/
			...
			// Any failure to respond here indicates a failure of the ASH or EZSP layers to initialize
			ncp.CurrentNCPVersion = ncp.Version(8);
			
			if (ncp.CurrentNCPVersion.ProtocolVersion is > Constants.EMBER_MAX_VERSION or < Constants.EMBER_MIN_VERSION)
			{
			...
			builder.Append("Ember Version=");
			builder.Append(ncp.CurrentNCPVersion.ProtocolVersion);
			builder.Append(", Stack Type=");
			builder.Append(ncp.CurrentNCPVersion.StackType);
			builder.Append(", Stack Version=");
			for (int cnt = 3; cnt >= 0; cnt--)
			{
				builder.Append((ncp.CurrentNCPVersion.StackVersion >> (cnt * 4)) & 0x0F);
				if (cnt != 0)
				{
					builder.Append('.');
				}
		...
		*/
		...
		{
			EmberNcp ncp = GetEmberNcp();
			Status concentratorResponse = ncp.SetConcentrator(
				(concentratorConfig.Type != ConcentratorType.DISABLED), 
				(ushort)concentratorConfig.Type, 
				concentratorConfig.MinTime, 
				concentratorConfig.MaxTime, 
				concentratorConfig.RouteErrorThreshold, 
				concentratorConfig.DeliveryFailureThreshold, 
				(byte)concentratorConfig.MaxHops);
			_logger.LogDebug(concentratorResponse.ToString());

			if (concentratorResponse == Status.SL_STATUS_OK)
				return ZigBeeStatus.SUCCESS;
		...
		*/
		private ZigBeeStatus SetEmberTxPower(sbyte txPower)
		{
			...
			EmberNcp ncp = GetEmberNcp();
			return (ncp.SetRadioPower(txPower) == Status.SL_STATUS_OK)
				? ZigBeeStatus.SUCCESS
