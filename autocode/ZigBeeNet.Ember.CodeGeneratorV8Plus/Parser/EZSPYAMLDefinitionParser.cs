        ...
        public void Process(string versionDir, string definitionPath, string versionName)
        {
            ...
            try
            {
                ...
                using (TextReader definitionFileReader = new StreamReader(definitionPath))
                {
                    ...
                    emberNCPAutoGeneratedContent.AppendLine("using ZigBeeNet.Hardware.EmberV8Plus.Ezsp.Common.Types;");
                    ...
                    foreach (FrameDefinition frameDefinition in sections.SelectMany(x => x.Frames ?? new List<FrameDefinition>()))
                    {
                        ...
                        if (frameDefinition.ResponseArguments.Count() > 1)
                        {
                            emberNCPAutoGeneratedContent.AppendLine($"\t\treturn ({String.Join(", ", frameDefinition.ResponseArguments.Select(arg =>
                            {
                                TypeMapping? argType = _typeMapperService.GetTypeMapping(arg.Type);
                                return $"response.{Sanitize.PropertyName(arg.Name)}";
                            }))});");
                        }
                        else if (frameDefinition.ResponseArguments.Count() == 1)
                        {
                            TypeMapping? argType = _typeMapperService.GetTypeMapping(frameDefinition.ResponseArguments[0].Type);
                            emberNCPAutoGeneratedContent.AppendLine($"\t\treturn response.{Sanitize.PropertyName(frameDefinition.ResponseArguments[0].Name)};");
                        }
                        else
                        {
