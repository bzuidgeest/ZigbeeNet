            ...
            try
            {
                ...
                using (TextReader definitionFileReader = new StreamReader(definitionPath))
                {
                    ...
                    foreach (FrameDefinition frameDefinition in sections.SelectMany(x => x.Frames ?? new List<FrameDefinition>()))
                    {
                        ...
                        {
                            string returnParameters = $"({String.Join(", ", frameDefinition.ResponseArguments.Select(arg => {
                        ...
                        if (frameDefinition.ResponseArguments.Count() >= 0)
                        {
                            string outputParameters = $"\t\treturn ({String.Join(", ", frameDefinition.ResponseArguments.Select(arg => {
                                TypeMapping? argType = _typeMapperService.GetTypeMapping(arg.Type);
                                return $"{sanitizedCommandName}Response.{arg.Name}";
                            }))});";
                        }
                        else
                        {
